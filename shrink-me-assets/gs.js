export const executeModule = async () => {
  ...
  // The Ghostscript object: Our interface to the outside world. We import
  // and export values on it. There are various ways Ghostscript can be used:
  // 1. Not defined. We create it here
  // 2. A function parameter, function(Ghostscript) { ..generated code.. }
  // 3. pre-run appended it, var Ghostscript = {}; ..generated code..
  // 4. External script tag defines var Ghostscript.
  // We need to check if Ghostscript already exists (e.g. case 3 above).
  // Substitution will be replaced with actual code on later stage of the build,
  // this way Closure Compiler will not mangle it (e.g. case 4. above).
  // Note that if you want to run closure, and also to use Ghostscript
  // after the generated code, you will need to define   var Ghostscript = {};
  // before the code. Then that object will be used in the code, and you
  // can continue to use Ghostscript afterwards as well.
  var Ghostscript =
    typeof window.Ghostscript !== 'undefined'
      ? window.Ghostscript
      : typeof Ghostscript !== 'undefined'
        ? Ghostscript
        : {}

  // --pre-jses are emitted after the Ghostscript integration code, so that they can
  // refer to Ghostscript (if they choose; they can also define Ghostscript)
  // {{PRE_JSES}}

  // Sometimes an existing Ghostscript object exists with properties
  ...
  let moduleOverrides = {}
  let key
  for (key in Ghostscript) {
    if (Ghostscript.hasOwnProperty(key)) {
      moduleOverrides[key] = Ghostscript[key]
    }
  }

  Ghostscript['arguments'] = []
  Ghostscript['thisProgram'] = './this.program'
  Ghostscript['quit'] = function (status, toThrow) {
    throw toThrow
  }
  Ghostscript['preRun'] = []
  Ghostscript['postRun'] = []
  ...
  if (Ghostscript['ENVIRONMENT']) {
    throw new Error(
      'Ghostscript.ENVIRONMENT has been deprecated. To force the environment, use the ENVIRONMENT compile-time option (for example, -s ENVIRONMENT=web or -s ENVIRONMENT=node)'
    )
  }
  ...
  function locateFile(path) {
    if (Ghostscript['locateFile']) {
      return Ghostscript['locateFile'](path, scriptDirectory)
    } else {
  ...
  if (ENVIRONMENT_IS_NODE) {
    ...
    Ghostscript['read'] = function shell_read(filename, binary) {
      let ret
    ...
    Ghostscript['readBinary'] = function readBinary(filename) {
      let ret = Ghostscript['read'](filename, true)
      if (!ret.buffer) {
    ...
    if (process['argv'].length > 1) {
      Ghostscript['thisProgram'] = process['argv'][1].replace(/\\/g, '/')
    }

    Ghostscript['arguments'] = process['argv'].slice(2)
    ...
    Ghostscript['quit'] = function (status) {
      process['exit'](status)
    }

    Ghostscript['inspect'] = function () {
      return '[Emscripten Ghostscript object]'
    }
  } else if (ENVIRONMENT_IS_SHELL) {
    if (typeof read != 'undefined') {
      Ghostscript['read'] = function shell_read(f) {
        return read(f)
      }
    }

    Ghostscript['readBinary'] = function readBinary(f) {
      let data
    ...
    if (typeof scriptArgs != 'undefined') {
      Ghostscript['arguments'] = scriptArgs
    } else if (typeof arguments != 'undefined') {
      Ghostscript['arguments'] = arguments
    }
    ...
    Ghostscript['read'] = function shell_read(url) {
    ...
    if (ENVIRONMENT_IS_WORKER) {
      Ghostscript['readBinary'] = function readBinary(url) {
        const xhr = new XMLHttpRequest()
        xhr.open('GET', url, false)
    ...
    Ghostscript['readAsync'] = function readAsync(url, onload, onerror) {
      const xhr = new XMLHttpRequest()
    ...
    Ghostscript['setWindowTitle'] = function (title) {
  ...
  // stderr, respectively.
  // If the user provided Ghostscript.print or printErr, use that. Otherwise,
  ...
  for (key in moduleOverrides) {
    if (moduleOverrides.hasOwnProperty(key)) {
      Ghostscript[key] = moduleOverrides[key]
    }
  }
  ...
  assert(
    typeof Ghostscript['memoryInitializerPrefixURL'] === 'undefined',
    'Ghostscript.memoryInitializerPrefixURL option was removed, use Ghostscript.locateFile instead'
  )
  assert(
    typeof Ghostscript['pthreadMainPrefixURL'] === 'undefined',
    'Ghostscript.pthreadMainPrefixURL option was removed, use Ghostscript.locateFile instead'
  )
  assert(
    typeof Ghostscript['cdInitializerPrefixURL'] === 'undefined',
    'Ghostscript.cdInitializerPrefixURL option was removed, use Ghostscript.locateFile instead'
  )
  assert(
    typeof Ghostscript['filePackagePrefixURL'] === 'undefined',
    'Ghostscript.filePackagePrefixURL option was removed, use Ghostscript.locateFile instead'
  )
  ...
  function dynamicAlloc(size) {
    ...
    if (end <= _emscripten_get_heap_size()) {
      ...
      const success = _emscripten_resize_heap(end)
      if (!success) return 0
    }
    return ret
  }
  ...
  function dynCall(sig, ptr, args) {
    if (args && args.length) {
      ...
      assert(
        'dynCall_' + sig in Ghostscript,
        "bad function pointer type - no table for sig '" + sig + "'"
      )
      return Ghostscript['dynCall_' + sig].apply(null, [ptr].concat(args))
    } else {
      assert(sig.length == 1)
      assert(
        'dynCall_' + sig in Ghostscript,
      ...
      return Ghostscript['dynCall_' + sig].call(null, ptr)
    }
  }
  ...
  function getCFunc(ident) {
    const func = Ghostscript['_' + ident] // closure exported function
  ...
  const ALLOC_DYNAMIC = 2 // Cannot be freed except through sbrk
  ...
  function stackTrace() {
    let js = jsStackTrace()
    if (Ghostscript['extraStackTrace']) js += '\n' + Ghostscript['extraStackTrace']()
  ...
  function updateGlobalBuffer(buf) {
    Ghostscript['buffer'] = buffer = buf
  }

  function updateGlobalBufferViews() {
    Ghostscript['HEAP8'] = HEAP8 = new Int8Array(buffer)
    Ghostscript['HEAP16'] = HEAP16 = new Int16Array(buffer)
    Ghostscript['HEAP32'] = HEAP32 = new Int32Array(buffer)
    Ghostscript['HEAPU8'] = HEAPU8 = new Uint8Array(buffer)
    Ghostscript['HEAPU16'] = HEAPU16 = new Uint16Array(buffer)
    Ghostscript['HEAPU32'] = HEAPU32 = new Uint32Array(buffer)
    Ghostscript['HEAPF32'] = HEAPF32 = new Float32Array(buffer)
    Ghostscript['HEAPF64'] = HEAPF64 = new Float64Array(buffer)
  }
  ...
  if (Ghostscript['TOTAL_STACK'])
    assert(
      TOTAL_STACK === Ghostscript['TOTAL_STACK'],
      'the stack size can no longer be determined at runtime'
    )

  let TOTAL_MEMORY = Ghostscript['TOTAL_MEMORY'] || 16777216
  ...
  if (Ghostscript['buffer']) {
    buffer = Ghostscript['buffer']
    assert(
    ...
    while (callbacks.length > 0) {
      ...
      if (typeof func === 'number') {
        if (callback.arg === undefined) {
          Ghostscript['dynCall_v'](func)
        } else {
          Ghostscript['dynCall_vi'](func, callback.arg)
        }
      } else {
  ...
  function preRun() {
    // compatibility - merge in anything from Ghostscript['preRun'] at this time
    if (Ghostscript['preRun']) {
      if (typeof Ghostscript['preRun'] == 'function')
        Ghostscript['preRun'] = [Ghostscript['preRun']]
      while (Ghostscript['preRun'].length) {
        addOnPreRun(Ghostscript['preRun'].shift())
      }
    }
  ...
  function postRun() {
    checkStackCookie()
    // compatibility - merge in anything from Ghostscript['postRun'] at this time
    if (Ghostscript['postRun']) {
      if (typeof Ghostscript['postRun'] == 'function')
        Ghostscript['postRun'] = [Ghostscript['postRun']]
      while (Ghostscript['postRun'].length) {
        addOnPostRun(Ghostscript['postRun'].shift())
      }
    }
  ...
  // Ghostscript.preRun (used by emcc to add file preloading).
  ...
  function addRunDependency(id) {
    runDependencies++
    if (Ghostscript['monitorRunDependencies']) {
      Ghostscript['monitorRunDependencies'](runDependencies)
    }
    if (id) {
  ...
  Ghostscript['preloadedImages'] = {} // maps url to image data
  Ghostscript['preloadedAudios'] = {} // maps url to audio data
  ...
  function getBinary() {
    try {
      if (Ghostscript['wasmBinary']) {
        return new Uint8Array(Ghostscript['wasmBinary'])
      }
      if (Ghostscript['readBinary']) {
        return Ghostscript['readBinary'](wasmBinaryFile)
      } else {
  ...
  function createWasm(env) {
    ...
    function receiveInstance(instance, module) {
      ...
      Ghostscript['asm'] = exports
      removeRunDependency('wasm-instantiate')
    }

    addRunDependency('wasm-instantiate')

    // User shell pages can write their own Ghostscript.instantiateWasm = function(imports, successCallback) callback
    // to manually instantiate the Wasm module themselves. This allows pages to run the instantiation parallel
    // to any other async startup actions they are performing.
    if (Ghostscript['instantiateWasm']) {
      try {
        return Ghostscript['instantiateWasm'](info, receiveInstance)
      } catch (e) {
        err('Ghostscript.instantiateWasm callback failed with error: ' + e)
        return false
      }
    }

    // Async compilation can be confusing when an error on the page overwrites Ghostscript
    // (for example, if the order of elements is wrong, and the one defining Ghostscript is
    // later), so we save Ghostscript and check it later.
    let trueModule = Ghostscript

    function receiveInstantiatedSource(output) {
      // 'output' is a WebAssemblyInstantiatedSource object which has both the module and instance.
      // receiveInstance() will swap in the exports (to Ghostscript.asm) so they can be called
      assert(
        Ghostscript === trueModule,
        'the Ghostscript object should not be replaced during async compilation - perhaps the order of HTML elements is wrong?'
      )
      trueModule = null
    ...
    function instantiateArrayBuffer(receiver) {
      ...
      fetchAndInstantiate(wasmBinaryFile).then(async (response) => {
    ...
    async function fetchAndInstantiate(url) {
      const moduleCache = await caches.open('wasm-module-cache')
      let response = await moduleCache.match(url)

      if (!response) {
        response = await fetch(url, { credentials: 'same-origin' })
        moduleCache.put(url, response.clone())
      }
    ...
    if (
      ...
      WebAssembly.instantiateStreaming(fetchAndInstantiate(wasmBinaryFile.default), info).then(
  ...
  Ghostscript['asm'] = function (global, env, providedBuffer) {
  ...
  function ___setErrNo(value) {
    if (Ghostscript['___errno_location']) HEAP32[Ghostscript['___errno_location']() >> 2] = value
      ...
      get_char: function (tty) {
        if (!tty.input.length) {
          ...
          if (ENVIRONMENT_IS_NODE) {
            ...
            const isPosixPlatform = process.platform != 'win32' // Node doesn't offer a direct check, so test by exclusion
    ...
    mount: function (mount) {
      ...
      ;(mount.opts['packages'] || []).forEach(function (pack) {
        pack['metadata'].files.forEach(function (file) {
          const name = file.filename.substr(1) // remove initial slash
          WORKERFS.createNode(
            ensureParent(name),
            base(name),
            WORKERFS.FILE_MODE,
  ...
  const _stdin = 10348704

  const _stdout = 10348720

  const _stderr = 10348736
  var FS = {
    root: null,
    mounts: [],
    devices: {},
    ...
    open: function (path, flags, mode, fd_start, fd_end) {
      ...
      if (Ghostscript['logReadFiles'] && !(flags & 1)) {
    ...
    createDefaultDevices: function () {
      ...
      // stderr needs to print output using Ghostscript['printErr']
    ...
    createSpecialDirectories: function () {
      ...
      FS.mount(
        {
          mount: function () {
            const node = FS.createNode('/proc/self', 'fd', 16384 | 511 /* 0777 */, 73)
            node.node_ops = {
              lookup: function (parent, name) {
                const fd = +name
                const stream = FS.getStream(fd)
    ...
    createStandardStreams: function () {
      ...
      // them instead.
      if (Ghostscript['stdin']) {
        FS.createDevice('/dev', 'stdin', Ghostscript['stdin'])
      } else {
        FS.symlink('/dev/tty', '/dev/stdin')
      }
      if (Ghostscript['stdout']) {
        FS.createDevice('/dev', 'stdout', null, Ghostscript['stdout'])
      } else {
        FS.symlink('/dev/tty', '/dev/stdout')
      }
      if (Ghostscript['stderr']) {
        FS.createDevice('/dev', 'stderr', null, Ghostscript['stderr'])
      } else {
    ...
    init: function (input, output, error) {
      ...
      // Allow Ghostscript.stdin etc. to provide defaults, if none explicitly passed to us here
      Ghostscript['stdin'] = input || Ghostscript['stdin']
      Ghostscript['stdout'] = output || Ghostscript['stdout']
      Ghostscript['stderr'] = error || Ghostscript['stderr']
    ...
    quit: function () {
      ...
      const fflush = Ghostscript['_fflush']
      if (fflush) fflush(0)
    ...
    forceLoadFile: function (obj) {
      ...
      } else if (Ghostscript['read']) {
        // Command-line.
        try {
          // WARNING: Can't read binary files in V8's d8 or tracemonkey's js, as
          //          read() will try to parse UTF8.
          obj.contents = intArrayFromString(Ghostscript['read'](obj.url), true)
    ...
    createPreloadedFile: function (
      ...
      function processData(byteArray) {
        ...
        let handled = false
        Ghostscript['preloadPlugins'].forEach(function (plugin) {
          if (handled) return
  ...
  function emscripten_realloc_buffer(size) {
    ...
    const old = Ghostscript['buffer']
    const oldSize = old.byteLength
    // native wasm support
    try {
      const result = wasmMemory.grow((size - oldSize) / 65536) // .grow() takes a delta compared to the previous size
      if (result !== (-1 | 0)) {
        // success in native wasm memory growth, get the buffer from the memory
        return (Ghostscript['buffer'] = wasmMemory.buffer)
      } else {
        return null
      }
    } catch (e) {
  ...
  function _emscripten_resize_heap(requestedSize) {
    ...
    let newSize = Math.max(oldSize, MIN_TOTAL_MEMORY) // So the loop below will not be infinite, and minimum asm.js memory size is 16MB.
  ...
  const ___tm_current = 10348784

  const ___tm_timezone = (stringToUTF8('GMT', 10348832, 4), 10348832)

  function _gmtime_r(time, tmPtr) {
    const date = new Date(HEAP32[time >> 2] * 1000)
  ...
  function _tzset() {
    ...
    const winter = new Date(2000, 0, 1)
    const summer = new Date(2000, 6, 1)
  ...
  function _localtime_r(time, tmPtr) {
    ...
    const yday = ((date.getTime() - start.getTime()) / (1000 * 60 * 60 * 24)) | 0
    HEAP32[(tmPtr + 28) >> 2] = yday
    HEAP32[(tmPtr + 36) >> 2] = -(date.getTimezoneOffset() * 60)

    // Attention: DST is in December in South, and some regions don't have DST at all.
    const summerOffset = new Date(2000, 6, 1).getTimezoneOffset()
  ...
  const _Int8Array = undefined

  const _Int32Array = undefined
  ...
  const _Math_floor = undefined

  const _Math_ceil = undefined
  ...
  __ATINIT__.unshift(function () {
    if (!Ghostscript['noFSInit'] && !FS.init.initialized) FS.init()
  })
  ...
  const real____emscripten_environ_constructor = asm['___emscripten_environ_constructor']
  ...
  const real___get_daylight = asm['__get_daylight']
  ...
  const real___get_environ = asm['__get_environ']
  ...
  const real___get_timezone = asm['__get_timezone']
  ...
  const real___get_tzname = asm['__get_tzname']
  ...
  const real__llvm_bswap_i16 = asm['_llvm_bswap_i16']
  ...
  const real__memmove = asm['_memmove']
  ...
  const real__pthread_mutex_lock = asm['_pthread_mutex_lock']
  ...
  const real__pthread_mutex_unlock = asm['_pthread_mutex_unlock']
  ...
  const real__realloc = asm['_realloc']
  ...
  const real__saveSetjmp = asm['_saveSetjmp']
  ...
  const real__setThrew = asm['_setThrew']
  ...
  const real__testSetjmp = asm['_testSetjmp']
  ...
  Ghostscript['asm'] = asm
  var ___emscripten_environ_constructor = (Ghostscript['___emscripten_environ_constructor'] =
    function () {
      assert(
        runtimeInitialized,
        'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
      ...
      return Ghostscript['asm']['___emscripten_environ_constructor'].apply(null, arguments)
    })
  const ___errno_location = (Ghostscript['___errno_location'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['___errno_location'].apply(null, arguments)
  })
  var __get_daylight = (Ghostscript['__get_daylight'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['__get_daylight'].apply(null, arguments)
  })
  const __get_environ = (Ghostscript['__get_environ'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['__get_environ'].apply(null, arguments)
  })
  var __get_timezone = (Ghostscript['__get_timezone'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['__get_timezone'].apply(null, arguments)
  })
  var __get_tzname = (Ghostscript['__get_tzname'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['__get_tzname'].apply(null, arguments)
  })
  const _emscripten_replace_memory = (Ghostscript['_emscripten_replace_memory'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['_emscripten_replace_memory'].apply(null, arguments)
  })
  const _fflush = (Ghostscript['_fflush'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['_fflush'].apply(null, arguments)
  })
  var _free = (Ghostscript['_free'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['_free'].apply(null, arguments)
  })
  const _llvm_bswap_i16 = (Ghostscript['_llvm_bswap_i16'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['_llvm_bswap_i16'].apply(null, arguments)
  })
  var _llvm_bswap_i32 = (Ghostscript['_llvm_bswap_i32'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['_llvm_bswap_i32'].apply(null, arguments)
  })
  const _main = (Ghostscript['_main'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['_main'].apply(null, arguments)
  })
  var _malloc = (Ghostscript['_malloc'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['_malloc'].apply(null, arguments)
  })
  const _memcpy = (Ghostscript['_memcpy'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['_memcpy'].apply(null, arguments)
  })
  const _memmove = (Ghostscript['_memmove'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['_memmove'].apply(null, arguments)
  })
  var _memset = (Ghostscript['_memset'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['_memset'].apply(null, arguments)
  })
  const _pthread_mutex_lock = (Ghostscript['_pthread_mutex_lock'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['_pthread_mutex_lock'].apply(null, arguments)
  })
  const _pthread_mutex_unlock = (Ghostscript['_pthread_mutex_unlock'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['_pthread_mutex_unlock'].apply(null, arguments)
  })
  const _realloc = (Ghostscript['_realloc'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['_realloc'].apply(null, arguments)
  })
  const _rintf = (Ghostscript['_rintf'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['_rintf'].apply(null, arguments)
  })
  const _saveSetjmp = (Ghostscript['_saveSetjmp'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['_saveSetjmp'].apply(null, arguments)
  })
  const _sbrk = (Ghostscript['_sbrk'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['_sbrk'].apply(null, arguments)
  })
  var _setThrew = (Ghostscript['_setThrew'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['_setThrew'].apply(null, arguments)
  })
  const _testSetjmp = (Ghostscript['_testSetjmp'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['_testSetjmp'].apply(null, arguments)
  })
  const establishStackSpace = (Ghostscript['establishStackSpace'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['establishStackSpace'].apply(null, arguments)
  })
  var stackAlloc = (Ghostscript['stackAlloc'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['stackAlloc'].apply(null, arguments)
  })
  var stackRestore = (Ghostscript['stackRestore'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['stackRestore'].apply(null, arguments)
  })
  var stackSave = (Ghostscript['stackSave'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['stackSave'].apply(null, arguments)
  })
  const dynCall_dd = (Ghostscript['dynCall_dd'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_dd'].apply(null, arguments)
  })
  const dynCall_ddd = (Ghostscript['dynCall_ddd'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_ddd'].apply(null, arguments)
  })
  const dynCall_diiid = (Ghostscript['dynCall_diiid'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_diiid'].apply(null, arguments)
  })
  const dynCall_fdd = (Ghostscript['dynCall_fdd'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_fdd'].apply(null, arguments)
  })
  const dynCall_fdi = (Ghostscript['dynCall_fdi'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_fdi'].apply(null, arguments)
  })
  const dynCall_fdii = (Ghostscript['dynCall_fdii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_fdii'].apply(null, arguments)
  })
  const dynCall_fiii = (Ghostscript['dynCall_fiii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_fiii'].apply(null, arguments)
  })
  const dynCall_i = (Ghostscript['dynCall_i'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_i'].apply(null, arguments)
  })
  const dynCall_id = (Ghostscript['dynCall_id'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_id'].apply(null, arguments)
  })
  const dynCall_idii = (Ghostscript['dynCall_idii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_idii'].apply(null, arguments)
  })
  var dynCall_ii = (Ghostscript['dynCall_ii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_ii'].apply(null, arguments)
  })
  const dynCall_iid = (Ghostscript['dynCall_iid'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iid'].apply(null, arguments)
  })
  const dynCall_iiddddddddi = (Ghostscript['dynCall_iiddddddddi'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iiddddddddi'].apply(null, arguments)
  })
  const dynCall_iiddddi = (Ghostscript['dynCall_iiddddi'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iiddddi'].apply(null, arguments)
  })
  const dynCall_iidi = (Ghostscript['dynCall_iidi'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iidi'].apply(null, arguments)
  })
  const dynCall_iidiii = (Ghostscript['dynCall_iidiii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iidiii'].apply(null, arguments)
  })
  var dynCall_iii = (Ghostscript['dynCall_iii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iii'].apply(null, arguments)
  })
  var dynCall_iiii = (Ghostscript['dynCall_iiii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iiii'].apply(null, arguments)
  })
  const dynCall_iiiid = (Ghostscript['dynCall_iiiid'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iiiid'].apply(null, arguments)
  })
  var dynCall_iiiii = (Ghostscript['dynCall_iiiii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iiiii'].apply(null, arguments)
  })
  const dynCall_iiiiii = (Ghostscript['dynCall_iiiiii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iiiiii'].apply(null, arguments)
  })
  const dynCall_iiiiiii = (Ghostscript['dynCall_iiiiiii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iiiiiii'].apply(null, arguments)
  })
  const dynCall_iiiiiiii = (Ghostscript['dynCall_iiiiiiii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iiiiiiii'].apply(null, arguments)
  })
  const dynCall_iiiiiiiifi = (Ghostscript['dynCall_iiiiiiiifi'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iiiiiiiifi'].apply(null, arguments)
  })
  const dynCall_iiiiiiiii = (Ghostscript['dynCall_iiiiiiiii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iiiiiiiii'].apply(null, arguments)
  })
  const dynCall_iiiiiiiiii = (Ghostscript['dynCall_iiiiiiiiii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iiiiiiiiii'].apply(null, arguments)
  })
  const dynCall_iiiiiiiiiii = (Ghostscript['dynCall_iiiiiiiiiii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iiiiiiiiiii'].apply(null, arguments)
  })
  const dynCall_iiiiiiiiiiii = (Ghostscript['dynCall_iiiiiiiiiiii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iiiiiiiiiiii'].apply(null, arguments)
  })
  const dynCall_iiiiiiiiiiiii = (Ghostscript['dynCall_iiiiiiiiiiiii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iiiiiiiiiiiii'].apply(null, arguments)
  })
  const dynCall_iiiiiiiiiiiiii = (Ghostscript['dynCall_iiiiiiiiiiiiii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iiiiiiiiiiiiii'].apply(null, arguments)
  })
  const dynCall_iiiiiiiiiiiiiii = (Ghostscript['dynCall_iiiiiiiiiiiiiii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iiiiiiiiiiiiiii'].apply(null, arguments)
  })
  const dynCall_iiiiiiiiiiiiiiii = (Ghostscript['dynCall_iiiiiiiiiiiiiiii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iiiiiiiiiiiiiiii'].apply(null, arguments)
  })
  const dynCall_iiiiiiiiiiiiiiiii = (Ghostscript['dynCall_iiiiiiiiiiiiiiiii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iiiiiiiiiiiiiiiii'].apply(null, arguments)
  })
  const dynCall_iiiiiiiiiiji = (Ghostscript['dynCall_iiiiiiiiiiji'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iiiiiiiiiiji'].apply(null, arguments)
  })
  const dynCall_iiiiiiiiiijj = (Ghostscript['dynCall_iiiiiiiiiijj'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iiiiiiiiiijj'].apply(null, arguments)
  })
  const dynCall_iiiiiiijjii = (Ghostscript['dynCall_iiiiiiijjii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iiiiiiijjii'].apply(null, arguments)
  })
  const dynCall_iiiiiij = (Ghostscript['dynCall_iiiiiij'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iiiiiij'].apply(null, arguments)
  })
  const dynCall_iiiiiiji = (Ghostscript['dynCall_iiiiiiji'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iiiiiiji'].apply(null, arguments)
  })
  const dynCall_iiiiijiii = (Ghostscript['dynCall_iiiiijiii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iiiiijiii'].apply(null, arguments)
  })
  const dynCall_iiiijii = (Ghostscript['dynCall_iiiijii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iiiijii'].apply(null, arguments)
  })
  const dynCall_iij = (Ghostscript['dynCall_iij'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iij'].apply(null, arguments)
  })
  const dynCall_iiji = (Ghostscript['dynCall_iiji'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iiji'].apply(null, arguments)
  })
  const dynCall_iijii = (Ghostscript['dynCall_iijii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iijii'].apply(null, arguments)
  })
  const dynCall_iijj = (Ghostscript['dynCall_iijj'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iijj'].apply(null, arguments)
  })
  const dynCall_iijjjjjj = (Ghostscript['dynCall_iijjjjjj'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iijjjjjj'].apply(null, arguments)
  })
  const dynCall_iji = (Ghostscript['dynCall_iji'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_iji'].apply(null, arguments)
  })
  const dynCall_ji = (Ghostscript['dynCall_ji'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_ji'].apply(null, arguments)
  })
  const dynCall_jii = (Ghostscript['dynCall_jii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_jii'].apply(null, arguments)
  })
  const dynCall_jiiiii = (Ghostscript['dynCall_jiiiii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_jiiiii'].apply(null, arguments)
  })
  const dynCall_jiji = (Ghostscript['dynCall_jiji'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_jiji'].apply(null, arguments)
  })
  const dynCall_jji = (Ghostscript['dynCall_jji'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_jji'].apply(null, arguments)
  })
  var dynCall_vi = (Ghostscript['dynCall_vi'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_vi'].apply(null, arguments)
  })
  var dynCall_vii = (Ghostscript['dynCall_vii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_vii'].apply(null, arguments)
  })
  var dynCall_viii = (Ghostscript['dynCall_viii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_viii'].apply(null, arguments)
  })
  var dynCall_viiii = (Ghostscript['dynCall_viiii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_viiii'].apply(null, arguments)
  })
  const dynCall_viiiii = (Ghostscript['dynCall_viiiii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_viiiii'].apply(null, arguments)
  })
  const dynCall_viiiiii = (Ghostscript['dynCall_viiiiii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_viiiiii'].apply(null, arguments)
  })
  const dynCall_viiiiiii = (Ghostscript['dynCall_viiiiiii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_viiiiiii'].apply(null, arguments)
  })
  const dynCall_viiiiiiii = (Ghostscript['dynCall_viiiiiiii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_viiiiiiii'].apply(null, arguments)
  })
  const dynCall_viiiiiiiii = (Ghostscript['dynCall_viiiiiiiii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_viiiiiiiii'].apply(null, arguments)
  })
  const dynCall_viiiiiiiiiiiiiiiiiiiiiiiiiiiiiiijiiiiii = (Ghostscript[
    ...
    return Ghostscript['asm']['dynCall_viiiiiiiiiiiiiiiiiiiiiiiiiiiiiiijiiiiii'].apply(
      null,
      arguments
    )
  })
  const dynCall_viiiiiiiiiiiiiijiiiii = (Ghostscript['dynCall_viiiiiiiiiiiiiijiiiii'] =
    function () {
      assert(
        runtimeInitialized,
        'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
      ...
      return Ghostscript['asm']['dynCall_viiiiiiiiiiiiiijiiiii'].apply(null, arguments)
    })
  const dynCall_viiiiiiiiijiiii = (Ghostscript['dynCall_viiiiiiiiijiiii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_viiiiiiiiijiiii'].apply(null, arguments)
  })
  const dynCall_vijii = (Ghostscript['dynCall_vijii'] = function () {
    assert(
      runtimeInitialized,
      'you need to wait for the runtime to be ready (e.g. wait for main() to be called)'
    )
    ...
    return Ghostscript['asm']['dynCall_vijii'].apply(null, arguments)
  })
  // === Auto-generated postamble setup entry stuff ===

  Ghostscript['asm'] = asm

  if (!Ghostscript['intArrayFromString'])
    Ghostscript['intArrayFromString'] = function () {
      abort(
        "'intArrayFromString' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)"
      )
    }
  if (!Ghostscript['intArrayToString'])
    Ghostscript['intArrayToString'] = function () {
      abort(
        "'intArrayToString' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)"
      )
    }
  if (!Ghostscript['ccall'])
    Ghostscript['ccall'] = function () {
      abort("'ccall' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)")
    }
  if (!Ghostscript['cwrap'])
    Ghostscript['cwrap'] = function () {
      abort("'cwrap' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)")
    }
  if (!Ghostscript['setValue'])
    Ghostscript['setValue'] = function () {
      abort("'setValue' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)")
    }
  if (!Ghostscript['getValue'])
    Ghostscript['getValue'] = function () {
      abort("'getValue' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)")
    }
  if (!Ghostscript['allocate'])
    Ghostscript['allocate'] = function () {
      abort("'allocate' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)")
    }
  if (!Ghostscript['getMemory'])
    Ghostscript['getMemory'] = function () {
      abort(
  ...
  if (!Ghostscript['removeRunDependency'])
    Ghostscript['removeRunDependency'] = function () {
      abort(
  ...
  if (!Ghostscript['ENV'])
    Ghostscript['ENV'] = function () {
      abort("'ENV' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)")
    }
  if (!Ghostscript['FS'])
    Ghostscript['FS'] = function () {
      abort("'FS' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)")
    }
  if (!Ghostscript['FS_createFolder'])
    Ghostscript['FS_createFolder'] = function () {
      abort(
  ...
  if (!Ghostscript['FS_createPath'])
    Ghostscript['FS_createPath'] = function () {
      abort(
  ...
  if (!Ghostscript['FS_createDataFile'])
    Ghostscript['FS_createDataFile'] = function () {
      abort(
  ...
  if (!Ghostscript['FS_createPreloadedFile'])
    Ghostscript['FS_createPreloadedFile'] = function () {
      abort(
  ...
  if (!Ghostscript['FS_createLazyFile'])
    Ghostscript['FS_createLazyFile'] = function () {
      abort(
  ...
  if (!Ghostscript['FS_createLink'])
    Ghostscript['FS_createLink'] = function () {
      abort(
  ...
  if (!Ghostscript['FS_createDevice'])
    Ghostscript['FS_createDevice'] = function () {
      abort(
  ...
  if (!Ghostscript['FS_unlink'])
    Ghostscript['FS_unlink'] = function () {
      abort(
  ...
  if (!Ghostscript['GL'])
    Ghostscript['GL'] = function () {
      abort("'GL' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)")
    }
  if (!Ghostscript['dynamicAlloc'])
    Ghostscript['dynamicAlloc'] = function () {
      abort(
        "'dynamicAlloc' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)"
      )
    }
  if (!Ghostscript['warnOnce'])
    Ghostscript['warnOnce'] = function () {
      abort("'warnOnce' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)")
    }
  if (!Ghostscript['loadDynamicLibrary'])
    Ghostscript['loadDynamicLibrary'] = function () {
      abort(
        "'loadDynamicLibrary' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)"
      )
    }
  if (!Ghostscript['loadWebAssemblyModule'])
    Ghostscript['loadWebAssemblyModule'] = function () {
      abort(
        "'loadWebAssemblyModule' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)"
      )
    }
  if (!Ghostscript['getLEB'])
    Ghostscript['getLEB'] = function () {
      abort("'getLEB' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)")
    }
  if (!Ghostscript['getFunctionTables'])
    Ghostscript['getFunctionTables'] = function () {
      abort(
        "'getFunctionTables' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)"
      )
    }
  if (!Ghostscript['alignFunctionTables'])
    Ghostscript['alignFunctionTables'] = function () {
      abort(
        "'alignFunctionTables' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)"
      )
    }
  if (!Ghostscript['registerFunctions'])
    Ghostscript['registerFunctions'] = function () {
      abort(
        "'registerFunctions' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)"
      )
    }
  if (!Ghostscript['addFunction'])
    Ghostscript['addFunction'] = function () {
      abort(
        "'addFunction' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)"
      )
    }
  if (!Ghostscript['removeFunction'])
    Ghostscript['removeFunction'] = function () {
      abort(
        "'removeFunction' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)"
      )
    }
  if (!Ghostscript['getFuncWrapper'])
    Ghostscript['getFuncWrapper'] = function () {
      abort(
        "'getFuncWrapper' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)"
      )
    }
  if (!Ghostscript['prettyPrint'])
    Ghostscript['prettyPrint'] = function () {
      abort(
        "'prettyPrint' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)"
      )
    }
  if (!Ghostscript['makeBigInt'])
    Ghostscript['makeBigInt'] = function () {
      abort("'makeBigInt' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)")
    }
  if (!Ghostscript['dynCall'])
    Ghostscript['dynCall'] = function () {
      abort("'dynCall' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)")
    }
  if (!Ghostscript['getCompilerSetting'])
    Ghostscript['getCompilerSetting'] = function () {
      abort(
        "'getCompilerSetting' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)"
      )
    }
  if (!Ghostscript['stackSave'])
    Ghostscript['stackSave'] = function () {
      abort("'stackSave' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)")
    }
  if (!Ghostscript['stackRestore'])
    Ghostscript['stackRestore'] = function () {
      abort(
        "'stackRestore' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)"
      )
    }
  if (!Ghostscript['stackAlloc'])
    Ghostscript['stackAlloc'] = function () {
      abort("'stackAlloc' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)")
    }
  if (!Ghostscript['establishStackSpace'])
    Ghostscript['establishStackSpace'] = function () {
      abort(
        "'establishStackSpace' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)"
      )
    }
  if (!Ghostscript['print'])
    Ghostscript['print'] = function () {
      abort("'print' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)")
    }
  if (!Ghostscript['printErr'])
    Ghostscript['printErr'] = function () {
      abort("'printErr' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)")
    }
  if (!Ghostscript['getTempRet0'])
    Ghostscript['getTempRet0'] = function () {
      abort(
        "'getTempRet0' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)"
      )
    }
  if (!Ghostscript['setTempRet0'])
    Ghostscript['setTempRet0'] = function () {
      abort(
        "'setTempRet0' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)"
      )
    }
  if (!Ghostscript['Pointer_stringify'])
    Ghostscript['Pointer_stringify'] = function () {
      abort(
        "'Pointer_stringify' was not exported. add it to EXTRA_EXPORTED_RUNTIME_METHODS (see the FAQ)"
      )
    }
  if (!Ghostscript['ALLOC_NORMAL'])
    Object.defineProperty(Ghostscript, 'ALLOC_NORMAL', {
      get: function () {
        abort(
  ...
  if (!Ghostscript['ALLOC_STACK'])
    Object.defineProperty(Ghostscript, 'ALLOC_STACK', {
      get: function () {
        abort(
  ...
  if (!Ghostscript['ALLOC_DYNAMIC'])
    Object.defineProperty(Ghostscript, 'ALLOC_DYNAMIC', {
      get: function () {
        abort(
  ...
  if (!Ghostscript['ALLOC_NONE'])
    Object.defineProperty(Ghostscript, 'ALLOC_NONE', {
      get: function () {
        abort(
  ...
  dependenciesFulfilled = function runCaller() {
    // If run has never been called, and we should call run (INVOKE_RUN is true, and Ghostscript.noInitialRun is not false)
    if (!Ghostscript['calledRun']) run()
    if (!Ghostscript['calledRun']) dependenciesFulfilled = runCaller // try this again later, after new deps are fulfilled
  }

  Ghostscript['callMain'] = function callMain(args) {
    assert(
      runDependencies == 0,
    ...
    HEAP32[argv >> 2] = allocateUTF8OnStack(Ghostscript['thisProgram'])
    ...
    try {
      const ret = Ghostscript['_main'](argc, argv, 0)
      ...
      if (e instanceof ExitStatus) {
        ...
        Ghostscript['noExitRuntime'] = true
        return
      } else {
        let toLog = e
  ...
  /** @type {function(Array=)} */
  function run(args) {
    args = args || Ghostscript['arguments']
    ...
    if (Ghostscript['calledRun']) return // run may have just been called through dependencies being fulfilled just in this very frame

    function doRun() {
      if (Ghostscript['calledRun']) return // run may have just been called while the async setStatus time below was happening
      Ghostscript['calledRun'] = true
      ...
      if (Ghostscript['onRuntimeInitialized']) Ghostscript['onRuntimeInitialized']()

      if (Ghostscript['_main'] && shouldRunNow) Ghostscript['callMain'](args)

      postRun()
    }

    if (Ghostscript['setStatus']) {
      Ghostscript['setStatus']('Running...')
      // setTimeout(function () {
      //   setTimeout(function () {
      //     Ghostscript['setStatus']('')
      //   }, 1)
      //   doRun()
      // }, 1)
  ...
  function exit(status, implicit) {
    ...
    if (implicit && Ghostscript['noExitRuntime'] && status === 0) {
      return
    }

    if (Ghostscript['noExitRuntime']) {
      ...
      if (Ghostscript['onExit']) Ghostscript['onExit'](status)
    }

    Ghostscript['quit'](status, new ExitStatus(status))
  }

  const abortDecorators = []

  function abort(what) {
    if (Ghostscript['onAbort']) {
      Ghostscript['onAbort'](what)
    }
  ...
  Ghostscript['abort'] = abort

  if (Ghostscript['preInit']) {
    if (typeof Ghostscript['preInit'] == 'function')
      Ghostscript['preInit'] = [Ghostscript['preInit']]
    while (Ghostscript['preInit'].length > 0) {
      Ghostscript['preInit'].pop()()
    }
  }

  // shouldRunNow refers to calling main(), not run().
  var shouldRunNow = true
  if (Ghostscript['noInitialRun']) {
    shouldRunNow = false
  }
